                ...
                .setPositiveButton("确认",new DialogInterface.OnClickListener(){
                    ...
                    public void onClick(DialogInterface arg0, int arg1) {
                        ...
                        if((s5=course_count.getText().toString()).equals("")||s1.equals("")) {
                            ...
                            for(int m=0;m<i;m++){
                                CourseTable.db.update(day,n+m+1,s1,s2,s3,s4,s5,s6,s7,Integer.toString(m));
                            }
                    ...
                    public void onClick(DialogInterface arg0, int arg1) {
                        ...
                        if(s5.equals("")||n1==Integer.parseInt(s5.trim())) {
                            switch(n2){
                                ...
                                case 1:
                                    CourseTable.db.update(day,n,s1,s2,s3,s4,s5,s6,s7,"0");
                                    for(int m=1;m<n1;m++){
                                        CourseTable.db.update(day,n+m,s1,s2,s3,s4,s5,s6,s7,Integer.toString(m));
                                    }
                                    break;
                                case 2:
                                    CourseTable.db.update(day,n-1,s1,s2,s3,s4,s5,s6,s7,"0");
                                    CourseTable.db.update(day,n,s1,s2,s3,s4,s5,s6,s7,"1");
                                    for(int m=2;m<n1;m++){
                                        CourseTable.db.update(day,n+m-1,s1,s2,s3,s4,s5,s6,s7,Integer.toString(m));
                                    }
                                    break;
                                case 3:
                                    for(int m=n2;m>=0;m--){
                                        CourseTable.db.update(day,n-m+1,s1,s2,s3,s4,s5,s6,s7,Integer.toString(n2-m));
                                    }
                                    break;
                            }
                        ...
                        //若节数有变化，先确定新节数并赋予旧的数据再更新数据
                            ...
                            if(n3<n1){
                                switch(n2){//删除
                                    case 0:
                                        for(int m=0;m<n1;m++){
                                            CourseTable.db.deleteData(day,n+m+1);
                                        }
                                        break;

                                    case 1:
                                        CourseTable.db.deleteData(day,n);
                                        for(int m=1;m<n1;m++){
                                            CourseTable.db.deleteData(day,n+m);
                                        }
                                        break;
                                    case 2:
                                        CourseTable.db.deleteData(day,n-1);
                                        CourseTable.db.deleteData(day,n);
                                        for(int m=2;m<n1;m++){
                                            CourseTable.db.deleteData(day,n+m-1);
                                        }
                                ...
                                switch(n2){//更新数据
                                    ...
                                    case 3:
                                        for(int m=0;m<n3;m++){
                                            CourseTable.db.update(day,n+m-2,s1,s2,s3,s4,s5,s6,s7,Integer.toString(m));
                                        }
                                        break;
                                }
